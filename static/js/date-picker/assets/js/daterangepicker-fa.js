  ...
  DateRangePicker.prototype = {
    ...
    updateMonthsInView: function () {
      if (this.endDate) {
        ...
        this.leftCalendar.month = jalaali
          ? this.startDate.clone().jDate(2)
          : this.startDate.clone().date(2);
        if (
          !this.linkedCalendars &&
          (this.endDate.month() != this.startDate.month() ||
            this.endDate.year() != this.startDate.year())
        ) {
          this.rightCalendar.month = jalaali
            ? this.endDate.clone().jDate(2)
            : this.endDate.clone().date(2);
        } else {
          this.rightCalendar.month = jalaali
            ? this.startDate.clone().jDate(2).add(1, "jMonth")
            : this.startDate.clone().date(2).add(1, "month");
        }
      } else {
        if (
          this.leftCalendar.month.format("YYYY-MM") !=
    ...
    renderCalendar: function (side) {
      ...
      if (
        ...
        html +=
          '<th class="prev available"><i class="icon-angle-left"></i></th>';
      } else {
        html += "<th></th>";
      }
      var dateHtml =
        this.locale.monthNames[
          jalaali ? calendar[1][1].jMonth() : calendar[1][1].month()
      ...
      if (this.showDropdowns) {
        ...
        for (var m = 0; m < 12; m++) {
          if (
            (!inMinYear || m >= minDate[localMonth]()) &&
            (!inMaxYear || m <= maxDate[localMonth]())
          ) {
            monthHtml +=
              "<option value='" +
              m +
      ...
      if (
        ...
        html +=
          '<th class="next available"><i class="icon-angle-right"></i></th>';
      } else {
        html += "<th></th>";
      }
      html += "</tr>";
      ...
      for (var row = 0; row < 6; row++) {
        ...
        for (var col = 0; col < 7; col++) {
          ...
          html +=
            ...
            '">' +
            renderPersianNumber(
              jalaali ? calendar[row][col].jDate() : calendar[row][col].date()
            ) +
            "</td>";
        }
        html += "</tr>";
