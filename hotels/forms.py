...
from .models import Request, Receipt
...
class BookingForm(forms.Form):
    firstname = forms.CharField(label='* نام', error_messages={'required': 'وارد کردن نام اجباری است.'},
                                widget=forms.TextInput(
                                    attrs={'id': 'firstname', 'class': 'form-control', 'placeholder': 'علی'}))
    lastname = forms.CharField(label='* نام خانوادگی', error_messages={'required': 'وارد کردن نام اجباری است.'},
                               widget=forms.TextInput(
                                   attrs={'id': 'lastname', 'class': 'form-control', 'placeholder': 'اوجی'}))
    email = forms.EmailField(required=False, label='ایمیل',
                             widget=forms.EmailInput(attrs={'class': 'form-control', 'placeholder': 'test@gmail.com'}))
    ...
    needs = forms.CharField(required=False, label='نیازمندی های ویژه', widget=forms.Textarea(
        attrs={'class': 'form-control', 'placeholder': 'ویلچر ، تخت اضافه...'}))
